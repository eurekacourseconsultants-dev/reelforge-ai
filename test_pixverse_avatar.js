...
;(async () => {
  ...
  // Type prompt
  const prompt = 'A pretty Asian woman, 22 years old, baby-faced, youthful smooth skin, chest-up portrait, facing directly at camera, sitting at a clean home office desk in a Singapore HDB flat, natural daylight from window, simple white walls, laptop visible in background, soft shallow depth of field with blurred background, calm confident smile, casual smart blouse, photorealistic'
  ...
  page.on('response', async response => {
    ...
    if (url.includes('media.pixverse.ai') && url.includes('t2i') && !url.includes('icon') && !url.includes('logo')) {
      ...
      try {
        ...
        const outPath = path.join(outDir, 'hdb-office-female-20s-01.jpg')
        fs.writeFileSync(outPath, buffer)
        console.log(`>>> CONVERTED WEBP -> JPEG, SAVED TO: ${outPath}`)
        const r2Url = await uploadToR2('avatars/hdb-office-female-20s-01.jpg', outPath, 'image/jpeg')
